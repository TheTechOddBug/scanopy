		...
		if (topology && (topology.edges || topology.nodes)) {
			...
			if (needsElk) {
				...
				if (isViewTransition && cachedSizes) {
					...
					console.log(
						`[CACHE] elemHits=${elemHits} elemMisses=${elemMisses} containerHits=${cacheHits} containerMisses=${cacheMisses} cacheSize=${containerSizeCache.size} viewCache=${viewCache?.size ?? 0}`
					);
				...
				if (allRootCollapsed && currentView !== 'Workloads') {
					...
					const forceEdgeHandles = new Map<string, import('../../layout/elk-layout').EdgeHandles>();
