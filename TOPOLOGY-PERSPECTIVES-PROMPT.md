...
1. **UX Design Doc**: `/Users/maya/.claude/projects/-Users-maya-dev-scanopy/planned-work/topology-visualization-redesign.md` — The full UX plan. Key sections: four perspectives, Perspective × C4 matrix, container rules vs leaf rules, primary/overlay edge classification, Layout Engine (elkjs + d3-force), Dependencys.
2. **Project Instructions**: `/Users/maya/dev/scanopy/CLAUDE.md`
3. **Architecture Audit**: `/Users/maya/.claude/plans/toasty-plotting-kahn.md` — Detailed audit of what needs fixing, with file paths, line numbers, and recommended fixes. **Read this carefully** — it's the basis for Batch 1. Note: the audit was written before a naming normalization pass, so some type names may have changed. Verify against current code.

## Current State

L3 topology is working with:
- Generalized node types (Container / Element) with metadata-driven frontend rendering
- GroupingConfig with container rules and element rules (independent dimensions)
...
- A naming normalization pass was just completed

There may be leftover worktrees from prior work — clean these up if they're already merged.
...
**Important:** The audit was written before a naming normalization. Verify these locations against the current codebase — type names and file structure may have shifted. The issues themselves are still valid.
...
Restructure TopologyRequestOptions so options are stored per-perspective:
...
Rename ELK-specific input/output types to generic `LayoutInput`/`LayoutResult`. Wrap current elkjs code in `ElkLayoutEngine`. `BaseTopologyViewer` calls `engine.compute()` without knowing which engine. Engine selected by perspective.
...
## Batch 2: Application Perspective + Dependencys
...
- Forces the Dependency rework (Groups → Dependencys), which needs to happen regardless
...
- **Primary edges:** Dependency edges (RequestPath, HubAndSpoke)
- **Overlay edges:** ServiceVirtualization (which services are containerized)
- **Layout hints:** Service categories could have a natural ordering, or no layer hints letting elkjs optimize freely

### 2b. Dependency rework

Rename "Groups" → "Dependencys" throughout UI and API:
...
- Dependencys are created/managed from the Application perspective (or a global management view)
...
- Application perspective shows services grouped by category with Dependency edges
- Dependencys can be created at service level (not just port binding level)
- UI says "Dependencys" everywhere, not "Groups"
...
- New element type variants (Port, Host-as-element) with resolver metadata
...
3. Clean up any leftover worktrees from prior work (check for unmerged changes first per CLAUDE.md)
4. Break Batch 1 into parallelizable worktree tasks. Likely:
